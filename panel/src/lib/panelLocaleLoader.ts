...
/** Supported panel locale codes — keep in sync with shared/localeMap.ts */
export const PANEL_LOCALE_CODES = [
    'ar',
    'bg',
    'bs',
    'cs',
...
export type PanelLocaleCode = (typeof PANEL_LOCALE_CODES)[number];

const panelLocaleCodeSet = new Set<string>(PANEL_LOCALE_CODES);

const isPanelLocaleCode = (code: string): code is PanelLocaleCode => panelLocaleCodeSet.has(code);

const localeImportByCode: Record<
    Exclude<PanelLocaleCode, 'en'>,
...
export const normalizePanelLangCode = (lang: string | undefined): string => {
    if (!lang) return 'en';
    if (lang === 'custom') return 'custom';
    if (isPanelLocaleCode(lang)) return lang;
    const baseLang = lang.split(/[-_]/)[0].toLowerCase();
    if (isPanelLocaleCode(baseLang)) return baseLang;
    return 'en';
};

/**
 * Loads a panel locale JSON on demand and merges it over English fallback strings.
 */
export const loadPanelLocale = async (lang: string): Promise<LocaleType> => {
    const normalized = normalizePanelLangCode(lang);
    if (normalized === 'en' || normalized === 'custom') return englishLocale;

    const cached = loadedLocales.get(normalized);
    if (cached) return cached;

    const importer = localeImportByCode[normalized as Exclude<PanelLocaleCode, 'en'>];
